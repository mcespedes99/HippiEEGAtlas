...
import pandas as pd
import numpy as np
from utils import extract_info, df_per_region, plotAllChannels_time, plotAllChannels_plotly, errorModifiedZScore, save_results_clustering, plotPaperFigures
...
edf_files = edf_files_layout.get(session = ['004', '007', '010', '012', '014'], extension='edf', return_type='filename')
edf_images = edf_files_layout.get(session = ['004', '007', '010', '012', '014'], extension='edf')
...
for region in psd_regions:
    ...
    if not os.path.exists(out_path_region):
        ...
        plotPaperFigures(f[0,:].squeeze(), psd, out_path=os.path.join(out_path_region, 'freq_domain_distribution.png'), output=True)
